...
def parse_args():
    ...
    mail_group.add_argument('--from', dest='from_addr', type=str, required=True, help='发件人地址')
    mail_group.add_argument('--from-name', type=str, help='发件人名称')
    mail_group.add_argument('--to', dest='to_addrs', type=str, required=True, help='收件人地址，多个地址用逗号分隔')
    mail_group.add_argument('--cc', dest='cc_addrs', type=str, help='抄送地址，多个地址用逗号分隔')
    mail_group.add_argument('--bcc', dest='bcc_addrs', type=str, help='密送地址，多个地址用逗号分隔')
    ...
    mail_group.add_argument('--attachment', action='append', help='附件文件路径，可多次使用')
    mail_group.add_argument('--priority', choices=['low', 'normal', 'high'], default='normal', help='邮件优先级')
...
def main():
    ...
    if args.load:
        try:
            email = MIMEHandler.parse_eml_file(args.load)
            logger.info(f"已从文件加载邮件: {args.load}")
        except Exception as e:
            logger.error(f"加载邮件失败: {e}")
            sys.exit(1)
    else:
        # 从命令行参数创建邮件
    ...
    if args.save:
        try:
            MIMEHandler.save_as_eml(email, args.save)
            logger.info(f"已保存邮件为: {args.save}")
    ...
    smtp_params['port'] = args.port or config.get('port') or SMTP_SERVER['port']
    smtp_params['use_ssl'] = args.ssl or config.get('use_ssl') or SMTP_SERVER['use_ssl']
    smtp_params['ssl_port'] = args.ssl_port or config.get('ssl_port') or SMTP_SERVER['ssl_port']
    ...
    try:
        ...
        print(f"邮件已成功发送到 {', '.join([addr.address for addr in email.to_addrs])}")
